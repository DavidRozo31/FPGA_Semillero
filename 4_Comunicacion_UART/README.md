...
### Codigo Completo del Transmitter:
...
architecture UART_Transmitter_arch of UART_Transmitter is
	...
	constant bauds : integer := 5208;
	signal counter : integer range 0 to bauds-1 := 0;
	signal bits : integer range 0 to 9 := 0;
...
architecture UART_Receiver_arch of UART_Receiver is
    ...
    constant BAUDS     : integer := 5208;
    constant HALF_BAUD : integer := BAUDS / 2;
    ...
    begin
        if rising_edge(clk) then
            if rst = '1' then
                ...
                -- rx_flag es pulso de 1 ciclo, bajar por defecto
                rx_flag <= '0';
                ...
                case state is
                    ...
                    -- ─────────────────────────────────────────
                    -- START: esperar medio periodo para centrar
                    -- el muestreo en el primer bit de datos
                    -- ─────────────────────────────────────────
                    when START =>
                        if counter = HALF_BAUD - 1 then
                            counter <= 0;
                    ...
                    -- ─────────────────────────────────────────
                    -- DATA: muestrear 8 bits, LSB primero
                    -- cada bit se lee en el centro del periodo
                    -- ─────────────────────────────────────────
                    when DATA =>
                        if counter = BAUDS - 1 then
                            counter              <= 0;
                            shift_reg(bit_idx)   <= rx;   -- LSB primero
                            if bit_idx = 7 then
                                bit_idx <= 0;
                                state   <= STOP;
                            else
                    ...
                    -- ─────────────────────────────────────────
                    -- STOP: verificar stop bit y emitir dato
                    -- ─────────────────────────────────────────
                    when STOP =>
                        if counter = BAUDS - 1 then
                            counter <= 0;
                            if rx = '1' then          -- stop bit válido
                                rx_data <= shift_reg;
                                rx_flag <= '1';       -- pulso 1 ciclo
                            end if;
                            state <= IDLE;            -- siempre volver a IDLE
                        else
                            counter <= counter + 1;
                        end if;
